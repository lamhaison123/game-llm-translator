...
def _plugins_js_path(game_dir: Path) -> Path | None:
    for candidate in (game_dir / "www" / "js" / "plugins.js", game_dir / "js" / "plugins.js"):
        if candidate.exists():
            return candidate
    return None


def _parse_plugins_js(text: str) -> Any:
    """Strip `var $plugins =` prefix + trailing `;` and parse as JSON.

    plugins.js is generated by RPG Maker MV/MZ as `var $plugins = [...];`.
    The bracketed body is valid JSON.
    """
    ...
    while stripped.startswith("//") or stripped.startswith("/*"):
        if stripped.startswith("//"):
            ...
            stripped = stripped[newline + 1:].lstrip() if newline >= 0 else ""
        else:
            end = stripped.find("*/")
            stripped = stripped[end + 2:].lstrip() if end >= 0 else ""
    if not stripped.startswith("var "):
        raise ValueError("plugins.js does not start with 'var $plugins ='")
    eq = stripped.find("=")
    if eq < 0:
        raise ValueError("plugins.js missing '=' after var declaration")
    body = stripped[eq + 1:].strip()
    if body.endswith(";"):
        body = body[:-1].rstrip()
    return json.loads(body)


def _extract_plugins_js(plugins_file: Path) -> list[TextEntry]:
    """Walk plugins.js entries extracting translatable strings from `parameters`.

    Each entry: {"name": "...", "parameters": {key: value, ...}}.
    Parameter values can be raw strings or JSON-encoded strings (Yanfly/MZ style).
    """
    entries: list[TextEntry] = []
    try:
        raw = plugins_file.read_text(encoding="utf-8-sig")
        data = _parse_plugins_js(raw)
    except (ValueError, json.JSONDecodeError, OSError) as exc:
        log_event(f"SKIP {plugins_file.name}: parse error ({exc})", level="WARN")
        return entries
    ...
    for idx, plugin in enumerate(data):
        ...
        for key, value in params.items():
            if not isinstance(value, str) or not _is_translatable_ui_text(value):
                continue
            # Some values are JSON-encoded strings (e.g. "[\"a\",\"b\"]"). Try unwrap.
            unwrapped = _try_unwrap_json_param(value)
            for sub_key, sub_value in unwrapped:
                if _is_translatable_ui_text(sub_value):
                    full_key = f"$[{idx}].parameters[{key!r}]"
                    if sub_key:
                        full_key += sub_key
                    entries.append(TextEntry(
                        file=plugins_file,
                        key=full_key,
                        source=sub_value,
                        context=f"rpg_maker_plugin_param_{plugin_name}",
                    ))
    return entries


def _try_unwrap_json_param(value: str) -> list[tuple[str, str]]:
    """Yield (sub_key, leaf_string) pairs for a plugin parameter value.

    Some plugins (Yanfly, VisuStella) encode array/dict parameters as JSON strings.
    Try to decode; on failure, treat the value as a single plain string.
    """
...
def extract_rpg_maker_json(game_dir: Path, plugin_text_extractor=None) -> tuple[list[TextEntry], list[str]]:
    ...
    plugins_file = _plugins_js_path(game_dir)
    if plugins_file is not None:
        entries.extend(_extract_plugins_js(plugins_file))
        ...
        if char == ".":
            ...
            if end == -1:
                ...
                try:
                    ...
                    # Strip surrounding quotes for string subscript like ['key name']
                    if len(idx_str) >= 2 and idx_str[0] == idx_str[-1] and idx_str[0] in ("'", '"'):
                        idx_str = idx_str[1:-1]
...
def _apply_plugins_js(plugins_file: Path, results: list[TranslationResult]) -> None:
    """Apply translations back to plugins.js in-place (same as Translator++ behavior)."""
    raw = plugins_file.read_text(encoding="utf-8-sig")
    data = _parse_plugins_js(raw)
    invalid: list[str] = []
    applied = 0
    for result in results:
        ...
        if warning is not None:
            invalid.append(warning)
        else:
            applied += 1
    if invalid:
        for w in invalid:
            log_event(f"WARN {plugins_file.name}: {w}", level="WARN")
    if applied == 0 and invalid:
        return
    # Preserve original header comment lines before `var $plugins`
...
def apply_rpg_maker(results: list[TranslationResult], output_dir: Path) -> None:
    ...
    for file in grouped:
        if file.name == "plugins.js":
            continue  # written in-place, not under output_dir
    ...
    for file, file_results in grouped.items():
        if file.name == "plugins.js":
            _apply_plugins_js(file, file_results)
            continue
