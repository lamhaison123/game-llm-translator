...
def install_xunity(
    ...
    _write_xunity_config(game_dir, target_lang, from_lang, progress)
...
def _write_xunity_config(game_dir: Path, target_lang: str, from_lang: str = "ja", progress: Callable[[str], None] | None = None) -> None:
    """Write XUnity AutoTranslatorConfig.ini with correct language ISO codes.

    Always overwrites so re-running with a different language works.
    """
    config_path = game_dir / "BepInEx" / "config" / "AutoTranslatorConfig.ini"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    lang_code = resolve_xunity_lang(target_lang or "vi")
    from_code = resolve_xunity_lang(from_lang or "ja")
    config_content = f"""[General]
Language={lang_code}
FromLanguage={from_code}
...
"""
    ...
    if progress:
        progress(f"XUnity config: {from_code} -> {lang_code} -> {config_path}")
